...
IMPORTANT: If you configure high availability, you might also want to configure the service level guaranteed. This is only available on clusters with the service level guaranteed.
[serviceLevel,yaml]
----
apiVersion: vshn.appcat.vshn.io/v1
kind: VSHNMariaDB
...
=== Custom ProxySQL Resources

You can override the default CPU and memory resources for ProxySQL in your claim.
Any unset field falls back to the defaults shown above.
