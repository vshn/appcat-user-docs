...
By default, the _WALs_ (Write Ahead Logs) will be archived continuously as they are completed.
This results in variable https://en.wikipedia.org/wiki/IT_disaster_recovery#Recovery_Point_Objective[RPO] times.

If a fixed RPO time is required, the `archive_timeout` setting can be set to force a WAL archive at the given interval:
