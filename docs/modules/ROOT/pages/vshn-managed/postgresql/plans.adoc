...
| standard-2 | 400m | 1936Mi | 20Gi
| standard-4 | 900m | 3984Mi | 40Gi
| standard-8 | 1900m | 8080Mi | 80Gi
|===
...
include::partial$disk-size-increase-only.adoc[]
...
When specifying custom CPU or memory limits you need to be aware that there is an additional resource overhead that depends on the deployment method.
See section <<_sidecar_overhead>>.

This means the actual resource consumption will be higher than what you configure. See section <<_sidecar_overhead>> for the specific overhead values.
...
The sidecar overhead depends on the deployment method.

=== CloudNativePG (default)

Each CloudNativePG-based instance has the following overhead:

* CPU: 100m
* Memory: 100Mi

This overhead is for the CNPG instance manager that runs alongside the PostgreSQL instance.
...
Each StackGres-based instance has the following overhead:
...
==== Dedicated Patroni Resources

By default, StackGres automatically distributes the specified resources across all containers (including Patroni and sidecars).
However, you can enable dedicated resource allocation for Patroni by setting `spec.parameters.service.dedicatedPatroniResources` to `true`.

When this option is enabled:

* The specified CPU and memory values are reserved exclusively for the Patroni container
* Sidecar containers receive separate resource allocations on top of the Patroni resources
* This increases the total sidecar overhead beyond the default 390m CPU and 408Mi memory
...
spec:
  ...
  parameters:
    size:
      plan: standard-4
    service:
      dedicatedPatroniResources: true # <1>
  writeConnectionSecretToRef:
    name: postgres-creds
----
<1> Enable dedicated resource allocation for Patroni, increasing the sidecar overhead.

IMPORTANT: Enabling `dedicatedPatroniResources` will increase the total resource consumption and cost of your PostgreSQL instance, as sidecar containers will receive additional resources on top of your configured values.
