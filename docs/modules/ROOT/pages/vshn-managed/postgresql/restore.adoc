...
CloudNativePG restores do not require specifying a base backup. The system automatically selects the appropriate base backup based on the archived WAL files.

. Apply the following object on your namespace, as specified by its YAML description.
+
.Example to restore a CloudNativePG PostgreSQL instance. Update the namespace!
[source,yaml]
----
...
spec:
  parameters:
    restore:
      ...
      recoveryTimeStamp: "2023-03-01T17:30:00+00:00" # <2>
    backup:
      schedule: '0 22 * * *'
    service:
      majorVersion: "16"
...
<2> Optional: Point-in-time recovery timestamp. If omitted, restores to the latest available WAL that has been archived.
...
. Apply the following object on your namespace, as specified by its YAML description.
+
.Example to restore a StackGres PostgreSQL instance. Update the namespace!
[source,yaml]
----
apiVersion: vshn.appcat.vshn.io/v1
kind: VSHNPostgreSQL
