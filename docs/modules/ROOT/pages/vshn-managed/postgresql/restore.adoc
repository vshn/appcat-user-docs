...
==== Restoring Databases with special permissions

If you used additional users and have non standard permissions in your database,
the user specified in the secret might not have the permission for this operation.
In this case you need to perform the dump and restore with the postgres user.

Run the script with the following parameters:

[source,bash]
----
./restore-single-db-with-postgres-user.sh <SECRET_NAME_FROM> <SECRET_NAME_TO> <NAMESPACE> <DATABASE_NAME>
----

Where:

* `SECRET_NAME_FROM`: The secret name for the postgres user of the temporary instance containing the restored backup
* `SECRET_NAME_TO`: The secret name for the postgres user of the target instance where you want to restore the database
* `NAMESPACE`: The namespace where both secrets for the postgres users are.
...
is_openshift=false
if kubectl get project &> /dev/null; then # 'project' resource is OpenShift specific
    is_openshift=true
fi
...
if [[ -z "$DATABASE_NAME" ]]; then
    read -rp "Enter the DATABASE_NAME to perform the action for: " DATABASE_NAME
fi
if [[ -z "$DATABASE_NAME" ]]; then
...
echo "Running pg_dump from pod '$POD_NAME' for database '$DATABASE_NAME'..."

echo "Tables and their stats in old db"
ka exec "$POD_NAME" --namespace="$NAMESPACE" -- \
  sh -c "PGPASSWORD=\${POSTGRESQL_PASSWORD} PGSSLMODE=disable psql -h \${POSTGRESQL_HOST} -U \${POSTGRESQL_USER} -d $DATABASE_NAME --tuples-only --command \
  \"ANALYZE;SELECT relname AS table_name, n_live_tup AS row_estimate FROM pg_stat_user_tables ORDER BY n_live_tup DESC;\""

ka exec "$POD_NAME" --namespace="$NAMESPACE" -- \
    sh -xc "PGPASSWORD=\${POSTGRESQL_PASSWORD} PGSSLMODE=disable pg_dump -h \${POSTGRESQL_HOST} -U \${POSTGRESQL_USER} -d $DATABASE_NAME --clean --if-exists 2>/dev/null \
       | PGPASSWORD=\${TO_POSTGRESQL_PASSWORD} PGSSLMODE=disable psql -h \${TO_POSTGRESQL_HOST} -U \${TO_POSTGRESQL_USER} -d $DATABASE_NAME"

echo "Tables and their stats in new db"
ka exec "$POD_NAME" --namespace="$NAMESPACE" -- \
  sh -c "PGPASSWORD=\${TO_POSTGRESQL_PASSWORD} PGSSLMODE=disable psql -h \${TO_POSTGRESQL_HOST} -U \${TO_POSTGRESQL_USER} -d $DATABASE_NAME --tuples-only --command \
    \"ANALYZE;SELECT relname AS table_name, n_live_tup AS row_estimate FROM pg_stat_user_tables ORDER BY n_live_tup DESC;\""
...
echo "Database '$DATABASE_NAME' successfully migrated"
