...
By default, Envoy sidecar proxy is disabled on PostgreSQL instances.
...
== CloudNativePG
CAUTION: Integration of this provider is under active development, it is not yet intended for use in production.
...
Similarly, existing instances cannot be switched over to CNPG without migrating to a new instance.
