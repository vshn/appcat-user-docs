...
These are the resources for NextCloud. For each NextCloud instance, a PostgreSQL Service with the same plan name and number of replicas is also instantiated. This can be customized using the postgreSQLParameters configuration field of the NextCloud service.
