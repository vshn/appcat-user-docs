...
.Nextcloud instance with custom maintenance
[source,yaml]
----
...
Since these jobs may decrease performance of the Nextcloud instance, we advise to set the maintenance window after work hours.
...
NOTE: During **major version upgrades**, app compatibility is the user's responsibility.
Some apps may not be compatible with a new Nextcloud major version.
See xref:vshn-managed/nextcloud/major-upgrade.adoc[Major Version Upgrade] for details.
