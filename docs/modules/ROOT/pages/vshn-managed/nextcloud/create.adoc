...
<3> Nextcloud version
<4> If we should use the default SQLite internal database instead of a dedicated PostgreSQL instance (defaults to false)
<5> PostgreSQL database parameters. See xref:vshn-managed/postgresql/index.adoc[PostgreSQL by VSHN] (Only used when `defaultInternalDB` is false)
