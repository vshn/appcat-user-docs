...
NOTE: Maintenance does not apply when `spec.parameters.service.customImage.image` is set.
In that case, automatic version upgrades are disabled and you manage the Keycloak version yourself via the image tag.
See xref:vshn-managed/keycloak/customization.adoc#_custom_keycloak_image_recommended[Custom Keycloak image] for details.
