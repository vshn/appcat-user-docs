...
NOTE: Updating an existing keycloak configuration can take up to 60 minutes to be applied.

== Themes, providers and custom files

`VSHNKeycloak` supports custom themes, providers, and files via custom container images.
See xref:vshn-managed/how-tos/custom-image-keycloak.adoc[How to create a custom Keycloak container image] for instructions on building the image.

=== Custom Keycloak image (recommended)

Use the `customImage` field to specify a full Keycloak image built from the https://github.com/vshn/custom-keycloak-image-template[custom-keycloak-image-template^].
Themes and providers are baked in at build time via `kc.sh build`, which is required for Keycloak's optimized startup mode.
The `customizationImage` approach copies files at runtime via init containers and is incompatible with optimized startup, this solution will be deprecated in the near future.

.Example: Keycloak instance with a custom image
[source,yaml]
----
...
spec:
  ...
  writeConnectionSecretToRef:
    name: keycloak-creds
----
<1> Full image reference for the custom Keycloak image built from the template.
<2> Optional: name of a Secret in the same namespace as the claim containing Docker registry credentials (key `.dockerconfigjson`).

NOTE: When `customImage` is set, automatic Keycloak version upgrades are disabled and maintenance windows do not apply.
The image tag is used as-is; you are responsible for updating the image when upgrading Keycloak.

=== Customization image

WARNING: The `customizationImage` field will be deprecated. Check `customImage` instead.
Setting both fields simultaneously is not allowed.

This approach uses an init container to copy themes and providers into the running Keycloak instance at pod startup.

.Example: Keycloak instance with a customization image (deprecated)
[source,yaml]
----
apiVersion: vshn.appcat.vshn.io/v1
kind: VSHNKeycloak
...
spec:
  parameters:
    service:
      version: "26"
      customizationImage:
        image: registry/user/image:tag # <1>
        imagePullSecretRef:
          name: secret-name # <2>
      customFiles:
        - source: path/in/customization/image # <3>
          destination: path/in/keycloak/instance # <4>
    size:
      plan: standard-2
  writeConnectionSecretToRef:
    name: keycloak-creds
...
<2> Secret name containing credentials for the registry with key `.dockerconfigjson` (must be in the same namespace as the claim)
<3> File in your customization image (can also be a folder)
<4> Destination path in Keycloak instance, relative to `/opt/keycloak`
