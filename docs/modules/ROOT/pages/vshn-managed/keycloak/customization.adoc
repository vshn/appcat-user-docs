...
The deprecated `customizationImage` approach copies files at runtime via init containers and is incompatible with optimized startup.
...
=== Customization image (deprecated)

WARNING: The `customizationImage` field is deprecated. Use `customImage` instead.
Setting both fields simultaneously is not allowed.

The legacy approach uses an init container to copy themes and providers into the running Keycloak instance at pod startup.
