...
| *`deletionPolicy`* __DeletionPolicy__ | DeletionPolicy determines how object buckets should be deleted. `DeleteAll` recursively deletes all objects in the object bucket and then removes it. `DeleteIfEmpty` only deletes the object bucket if it's empty.
|===
