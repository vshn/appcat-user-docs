...
[IMPORTANT]
====
For PostgreSQL instances, the major version in `pinImageTag` must match the `majorVersion` field.
...
[NOTE]
====
All services get patch version upgrades during maintenance except Keycloak and PostgreSQL that get minor version upgrades.
====
...
NOTE: For StackGres-based PostgreSQL instances (with `compositionRef: vshnpostgres.vshn.appcat.vshn.io`), `pinImageTag` is not supported.
