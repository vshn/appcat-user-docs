...
<3> The bucket's region depends on the selected Provider, see xref:object-storage/zones.adoc[here] for more details
...
spec:
  ...
  parameters:
    region: rma <2>
----
<1> The reference name of the backing cloud provider, see xref:object-storage/compositions.adoc[here] for available references.
<2> Change the region identifier in accordance for the resp. cloud provider, see xref:object-storage/zones.adoc[here] for more details.
